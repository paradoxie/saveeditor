...
function summarizeTier(results: SupportedCaseResult[], tier: Tier) {
    ...
    const roundTripPassed = tierResults.filter((item) => item.roundTripOk).length;

    return {
        total: tierResults.length,
        parseRate: ratio(parsePassed, tierResults.length),
        roundTripRate: ratio(roundTripPassed, tierResults.length),
    };
}
...
function printReport(
    ...
    supported.forEach((item) => {
        console.log(
            `  - ${item.id}: parse=${item.parseOk ? 'PASS' : 'FAIL'}, roundTrip=${item.roundTripOk ? 'PASS' : 'FAIL'} (${item.tier}, ${item.detail})`
        );
    });
