...
答：可以，但范围有限。当前支持稳定写回 `persistent` 下的基础类型字段；完整游戏状态 pickle 仍会被拦截。

**问：为什么不直接支持呢？**
答：在网络浏览器中执行任意 pickle 数据可能允许恶意代码执行。我们将用户安全置于功能完整性之上。

**问：您会支持完整编辑吗？**
答：不会承诺任意 pickle 全量编辑。当前稳定方向是带严格保护的 persistent 基础字段编辑。
...
我们的存档查看器帮助您了解存档状态；直接写回仅限 persistent 下的基础值。目前，开发者控制台仍然是 Ren'Py 官方生态中更可控的修改方式。

## 延伸阅读

- 📖 [Ren'Py 官方文档](https://www.renpy.org/doc/html/) - 引擎官方文档
- 🎮 [心跳文学部游戏页面](/zh-cn/games/ddlc) - 存档位置和可编辑项
- 📂 [常见存档扩展名详解](/zh-cn/blog/common-save-file-extensions-explained) - 了解各种格式
- 🔧 [Ren'Py 编辑器](/zh-cn/editor/renpy) - 存档查看工具
- 🎭 [NaniNovel 存档指南](/zh-cn/blog/naninovel-save-editing-guide) - 另一款视觉小说引擎
...
- [NaniNovel 存档编辑](/zh-cn/blog/naninovel-save-editing-guide)
