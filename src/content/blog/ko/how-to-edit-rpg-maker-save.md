...
RPG Maker MV 게임은 `.rpgsave` 파일 확장자를 사용하며, MZ 게임도 일반적으로 동일한 형식을 사용합니다. 이 파일들은 보통 LZString으로 압축되어 있어 일반 텍스트 편집기에서는 읽을 수 없습니다. 여기서 우리의 **SaveEditor.top**이 필요합니다 – 모든 압축 해제와 파싱을 브라우저에서 처리합니다.
...
기본 편집 흐름은 호환 세이브 파일을 **브라우저에서** 분석합니다.
...
### 골드
"골드 / 돈" 필드를 합리적인 테스트 값으로 변경한 뒤 게임 안에서 결과를 확인하세요.
